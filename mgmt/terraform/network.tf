...
module "vpc" {
  ...
  cidr              = data.aws_vpc_ipam_preview_next_cidr.preview_cidr.cidr
  ...
  depends_on = [
    aws_vpc_ipam_pool_cidr.mgmt
  ]
}

data "aws_vpc_ipam_preview_next_cidr" "preview_cidr" {
  ipam_pool_id = aws_vpc_ipam_pool.mgmt.id

  depends_on = [aws_vpc_ipam_pool_cidr.mgmt]
}

locals {
  ipam_cidr = data.aws_vpc_ipam_preview_next_cidr.preview_cidr.cidr
